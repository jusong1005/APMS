...
DEFAULT_PRODUCTS = [
    "番茄", "大白菜", "黄瓜", "土豆", "青椒", "尖椒", "茄子", "生菜", "菠菜", "萝卜", "胡萝卜", "西兰花", "菜花", "油菜", "芹菜", "豆角",
    "玉米", "小麦", "大米", "大豆", "高粱", "小米", "荞麦", "绿豆",
    "白条猪", "牛肉", "羊肉", "白条鸡", "鸡肉", "鸭肉", "鸡蛋",
]
PAGE_LIMIT = 200
REQUEST_TIMEOUT = 20
...
PRODUCT_ALIAS_HINTS = {
    ...
    "土豆": ["土豆", "马铃薯"],
    "青椒": ["青椒", "麻椒", "柿子椒", "彩椒"],
    "尖椒": ["尖椒", "绿尖椒", "红尖椒", "美人椒", "螺丝椒", "线椒", "杭椒"],
    "茄子": ["茄子", "圆茄子", "长茄子"],
    "生菜": ["生菜", "团生菜", "罗马生菜", "散叶生菜", "散生菜"],
    "菠菜": ["菠菜"],
    "萝卜": ["萝卜", "白萝卜", "青萝卜"],
    "胡萝卜": ["胡萝卜"],
    "西兰花": ["西兰花"],
    ...
    "苹果": ["苹果"],
    "猪肉": ["猪肉", "白条猪"],
    "牛肉": ["牛肉", "排酸牛肉", "牛腩", "牛前腱", "牛后腱", "牛柳"],
    "羊肉": ["羊肉", "整羊", "羊肉片", "羊排骨"],
    "鸡肉": ["鸡肉", "白条鸡", "肉鸡", "鸡胸", "鸡翅", "鸡腿"],
    "鸭肉": ["鸭肉", "白条湖鸭", "鸭腿", "鸭胗"],
    "鸡蛋": ["鸡蛋", "筐鸡蛋", "散鸡蛋", "柴鸡蛋", "乌鸡蛋", "红皮鸡蛋", "白皮鸡蛋"],
}
...
def collect_once(products: List[str], lookback_days: int, price_output: Path | None = None, weather_output: Path | None = None) -> Dict[str, object]:
    ...
    if price_output is not None:
        ...
        price_df.to_csv(price_output, index=False, encoding="utf-8-sig")
    ...
    return {
        ...
        "price_output": str(price_output) if price_output is not None else None,
        "weather_output": str(weather_output) if weather_output is not None else None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--price-output", type=Path, default=None, help="价格 CSV 输出路径")
    parser.add_argument("--weather-output", type=Path, default=None, help="天气 CSV 输出路径")
...
def main() -> int:
    args = parse_args()
    while True:
        summary = collect_once(args.products, args.lookback_days, args.price_output, args.weather_output)
